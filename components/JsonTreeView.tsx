...
const JsonTreeView = ({ data: initialData }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-800 text-slate-300 rounded-md font-mono text-sm">
            {/* Toolbar */}
            <div className="flex-shrink-0 p-3 bg-slate-900/50 rounded-t-md border-b border-slate-700">
                <div className="flex items-center gap-3">
                    <div className="relative flex-grow">
                        <input
                            ...
                            className="w-full bg-slate-700 border border-slate-600 rounded-md px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        />
                         {suggestions.length > 0 && (
                            <div className="absolute top-full left-0 right-0 mt-1 bg-slate-600 border border-slate-500 rounded-md shadow-lg z-10 text-slate-200">
                                {suggestions.map(suggestion => (
                                    <button
                                        key={suggestion}
                                        onMouseDown={() => handleSuggestionClick(suggestion)}
                                        className="w-full text-left px-3 py-2 hover:bg-slate-500 transition-colors"
                                    >
                                        {suggestion}
                                    </button>
                                ))}
                    ...
                    <button onClick={handleToggleSelectAll} className="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-md">
                        {allFieldsSelected ? 'Deselect All' : 'Select All'}
                    </button>
                    <button onClick={handleToggleExpandAll} className="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-md">
            ...
            <div className="flex-1 overflow-auto p-3 space-y-2">
                {Object.keys(filteredSections).length > 0 ? Object.entries(filteredSections).map(([sectionTitle, sectionData]) => (
                    <div key={sectionTitle} className="bg-slate-900/30 rounded-md border border-slate-700">
                        <button onClick={() => toggleSection(sectionTitle)} className="w-full flex items-center justify-between p-2 bg-slate-700/50 rounded-t-md">
                            <span className="font-bold text-blue-400">{sectionTitle}</span>
                        ...
                        {expandedSections.includes(sectionTitle) && (
                            <div className="p-2">
                                {Object.entries(sectionData).map(([key, value]) => {
                                    ...
                                    return (
                                        <div key={key} className={`flex items-center gap-3 p-1.5 rounded ${isSelected ? 'bg-indigo-500/20' : ''}`}>
                                            <input type="checkbox" checked={isSelected} onChange={() => setSelectedFields(p => p.includes(fieldPath) ? p.filter(i => i !== fieldPath) : [...p, fieldPath])} className="form-checkbox h-4 w-4 bg-slate-600 border-slate-500 rounded text-indigo-500 focus:ring-indigo-500" />
                                            <div className="w-1/3 truncate text-slate-400" title={key}>{key}</div>
                                            <div className="flex-1 bg-slate-700 rounded px-2 py-1 text-slate-200">
                ...
                )) : <div className="text-center text-slate-500 py-10">No data to display or matches found.</div>}
            </div>
        </div>
    );
};
